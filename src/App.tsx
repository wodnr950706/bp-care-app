...
const AlarmModal = ({
  ...
  alarm: AlarmSetting;
  onClose: () => void;
  onRecord: (medName: string, taken: boolean) => void;
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-slate-500 font-medium">약을 복용할 시간입니다!</p>
          </div>
        </div>

        <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
          <p className="text-xs font-bold text-slate-400 uppercase mb-2">오늘의 복용 현황</p>
          <div className="flex items-center justify-center gap-2">
            <span className="text-3xl font-black text-blue-600">{todayMedCount}</span>
            <span className="text-lg font-bold text-slate-900">회 복용함</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3">
          <button
            onClick={() => {
              onRecord(alarm.label, true);
              onClose();
            }}
            className="w-full bg-blue-600 text-white py-5 rounded-3xl font-bold text-lg shadow-xl shadow-blue-200 flex items-center justify-center gap-2"
          >
            <CheckCircle2 size={24} /> 지금 복용함
          </button>
          <button
            onClick={() => {
              onRecord(alarm.label, false);
              onClose();
            }}
            className="w-full bg-slate-100 text-slate-500 py-4 rounded-3xl font-bold text-sm"
          >
            나중에 기록하기
          </button>
        </div>
...
const HomeView = ({ 
  ...
  onNavigate: (s: Screen) => void, 
  records: HealthRecord[],
  onPrefillRecord: (p: { type: 'bloodSugar' | 'medication', medicationName?: string, category?: TimingCategory }) => void
}) => {
  const [today, setToday] = useState('');
...
<<<<<<< HEAD
  const todayAllRecords = records.filter((r) => r.dateString === todayStr);
  const todayMedicationCount = todayAllRecords.filter(
    (r) => r.type === 'medication' && r.taken
  ).length;

  const bloodSugarRecords = records.filter((r) => r.type === 'bloodSugar');
  const latestSugar =
    bloodSugarRecords.length > 0 ? bloodSugarRecords[0].bloodSugar : null;
=======
...
<<<<<<< HEAD
        ...
        <Card className="flex flex-col items-center justify-center text-center p-4">
          ...
          <p className="text-xl font-black text-slate-900">{todayAllRecords.length}</p>
          <p className="text-[10px] text-slate-400">약 복용 {todayMedicationCount}회</p>
=======
          <p className="text-xl font-black text-slate-900">{latestSugar ? `${latestSugar}` : '--'}</p>
          <p className="text-[10px] text-slate-400">mg/dL</p>
        </Card>
        <Card className="flex flex-col items-center justify-center text-center p-4">
          <div className="p-3 bg-emerald-50 text-emerald-600 rounded-2xl mb-2">
...
<<<<<<< HEAD
        ...
        >
=======
        <Card className="flex items-center justify-between py-4" onClick={() => {
          onPrefillRecord({ type: 'medication' });
          onNavigate('record');
        }}>
>>>>>>> cd40c366a9f636fbfb5d9f88880a11bf15f3d1c5
...
=======
        <button 
          onClick={() => setType('bloodSugar')}
          className={`flex-1 py-3 rounded-xl font-bold text-sm transition-all ${type === 'bloodSugar' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}
        >
          혈당 측정
        </button>
        <button 
          onClick={() => setType('medication')}
          className={`flex-1 py-3 rounded-xl font-bold text-sm transition-all ${type === 'medication' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}
>>>>>>> cd40c366a9f636fbfb5d9f88880a11bf15f3d1c5
        >
          약 복용
        </button>
...
<<<<<<< HEAD
            ...
            <ReferenceLine
              ...
              stroke="#10b981"
              strokeDasharray="3 3"
              label={{ value: '정상', position: 'right', fill: '#10b981', fontSize: 10 }}
            />
            <ReferenceLine
              y={140}
              stroke="#f59e0b"
              strokeDasharray="3 3"
              label={{ value: '주의', position: 'right', fill: '#f59e0b', fontSize: 10 }}
            />
            <Line
              type="monotone"
              dataKey="value"
...
const SettingsView = ({
  ...
  onSyncToSystem: (alarm: AlarmSetting) => void;
}) => {
...
<<<<<<< HEAD
          <button
            ...
            disabled={!name}
            className={`text-xs font-bold flex items-center gap-1 ${name ? 'text-rose-500' : 'text-slate-300 cursor-not-allowed'}`}
          >
...
<<<<<<< HEAD
            <br /><br />
            <strong>시계 앱 등록(<ExternalLink size={10} className="inline" />)</strong> 버튼을 누르면 휴대폰의 기본 시계 앱에 알람을 추가할 수 있습니다. (안드로이드 전용)
          </p>
        </div>
=======
          </p>
  ...
  useEffect(() => {
    const initNotifications = async () => {
      try {
        ...
        // Register action types for rich notifications
        ...
        await LocalNotifications.addListener(
          ...
          async (event) => {
            ...
            if (actionId === 'TAKEN') {
              ...
              setRecords((prev) =>
                [medicationRecord, ...prev].sort((a, b) => b.timestamp - a.timestamp)
              );
    ...
    const syncDeviceNotifications = async () => {
      try {
        ...
        for (const alarm of alarms) {
          ...
          const notificationId = alarm.id.split('').reduce((acc, ch) => acc + ch.charCodeAt(0), 0);
...
=======
  ...
  const [statusMessage, setStatusMessage] = useState<string>('');
  const [recordPrefill, setRecordPrefill] = useState<{ type: 'bloodSugar' | 'medication', medicationName?: string, category?: TimingCategory } | undefined>(undefined);
...
<<<<<<< HEAD
  ...
  const handleSyncToSystem = (alarm: AlarmSetting) => {
    const [hour, minute] = alarm.time.split(':').map(Number);
    // Android Intent URL for setting an alarm
    const intentUrl = `intent://#Intent;action=android.intent.action.SET_ALARM;i.android.intent.extra.HOUR=${hour};i.android.intent.extra.MINUTES=${minute};i.android.intent.extra.MESSAGE=${encodeURIComponent(alarm.label)};i.android.intent.extra.SKIP_UI=true;end`;
    
    window.location.href = intentUrl;
    setStatusMessage(`${alarm.label}이(가) 휴대폰 시계 앱에 등록되었습니다.`);
  };

  const getTodayMedCount = () => {
    const now = new Date();
    const todayStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
    return records.filter(r => r.dateString === todayStr && r.type === 'medication' && r.taken).length;
  };
...
<<<<<<< HEAD
      ...
      {activeAlarm && (
        <AlarmModal
          ...
          onRecord={(medName, taken) => handleSaveRecord({
            type: 'medication',
            medicationName: medName,
            taken,
            category: '약 복용',
          ...
          todayMedCount={getTodayMedCount()}
        />
      )}
        ...
        >
          {currentScreen === 'home' && (
            <HomeView 
              ...
              records={records} 
              onPrefillRecord={setRecordPrefill}
            />
          )}
...
{/* activeAlarm 기반 모달은 Local Notifications 전환 후 우선 사용 안 함 */}
...
<<<<<<< HEAD
          ...
          {currentScreen === 'settings' && (
            <SettingsView
              ...
              onToggleAlarm={handleToggleAlarm}
              onAlarmTimeChange={handleAlarmTimeChange}
              onSyncToSystem={handleSyncToSystem}
=======
            <HomeView 
              name={name} 
              onNavigate={setCurrentScreen} 
